...
pub async fn fetch_streak_data(
    username: &str,
) -> Result<(String, String, String), Box<dyn std::error::Error>> {
    let svg_html = reqwest::get(format!(
    ...
    let stats = {
        ...
        let total_contributions = get_text_content(
            &document,
            "svg > g > g:nth-child(3) > g:nth-child(1) > text",
        )
        .unwrap_or("None".to_string());
        let current_streak = get_text_content(
            &document,
            "svg > g > g:nth-child(4) > g:nth-child(5) > text",
        )
        .unwrap_or("None".to_string());
        let longest_streak = get_text_content(
            &document,
            "svg > g > g:nth-child(5) > g:nth-child(1) > text",
        )
        .unwrap_or("None".to_string());
